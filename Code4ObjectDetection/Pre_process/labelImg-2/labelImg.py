...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def loadFile(self, filePath=None):
        ...
        if unicodeFilePath and self.fileListWidget.count() > 0:
            if unicodeFilePath in self.mImgList:
                self.index = self.mImgList.index(unicodeFilePath)
                fileWidgetItem = self.fileListWidget.item(self.index)
        ...
        if unicodeFilePath and os.path.exists(unicodeFilePath):
            ...
            if self.defaultSaveDir is not None:
                ...
                if os.path.isfile(xmlPath):
                    self.loadPascalXMLByFilename(xmlPath, False)
                ...
                # add by Meepo start
                # if not find xmlpath, find the previous one, and show bound box.
                else:
                    ...
                    if self.index == 0:
                        ...
                        self.index -= 1
                        xmlPath = os.path.splitext(self.mImgList[self.index])[0] + XML_EXT
                        if os.path.isfile(xmlPath):
        ...
        if self.usingPascalVocFormat:
            ...
            if filename:
                if isinstance(filename, (tuple, list)):
                    filename = filename[0]
            self.loadPascalXMLByFilename(filename, False)
    ...
    def loadPascalXMLByFilename(self, xmlPath, is_previous):
        if self.filePath is None:
            return
