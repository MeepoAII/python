...
class NeuralNetwork:
    ...
    def feed_forward(self, inputs):  # 返回输出层y值
        ...
        out = self.output_layer.feed_forward(hidden_layer_outputs)
        # print("out:", out)
        return out
...
# build format train set
training_sets = np.random.uniform(0, 1, (1000, 2, 1))
for i in range(1000):
    training_sets[i][0][0] = x[i]
    training_sets[i][1][0] = y[i]

# write to file
# dataframe = pd.DataFrame({'x':x, 'y':y})
# dataframe.to_csv("trainingset.csv", index=False, sep=',')


plt.figure("figure")
ax = plt.gca()
ax.scatter(x, y, c='r', s=10, alpha=0.3)
plt.show()
...
for i in range(1000):
    ...
    this = nn.calculate_total_error(training_sets)
    steps.append(i)
    error.append(this)
    print("steps:", i, "error:", this)

plt.figure("error-steps")
ax = plt.gca()
ax.scatter(steps, error, c='r', s=10, alpha=0.3)
ax.plot(steps, error, c='b', linewidth=1, alpha=0.6)
plt.show()

# plt.figure("out-steps")
# ax = plt.gca()
# ax.scatter(steps, this_output, c='r', s=10, alpha=0.3)
# plt.show()
